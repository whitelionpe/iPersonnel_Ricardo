...
export const URL = Constants.API_URL + "/api/SistemaImportacionDetalle";
...
export const service = {
  ...
  obtenerDetalleTabla: (params) => {
    return from(axios.get(`${URL}/obtenerDetalleTabla`, { params })).pipe(
      map(result => result.data.result)
    ).toPromise();
  },

  crearDetalleTablaDesdeBD: (params) => {
    return from(axios.post(`${URL}/crearDetalleTablaDesdeBD`, params)).pipe(
      map(result => result.data.result)
    ).toPromise();
  },
