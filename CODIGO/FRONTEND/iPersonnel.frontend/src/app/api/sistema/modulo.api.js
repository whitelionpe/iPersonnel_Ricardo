...
export const URL = Constants.API_URL + "/api/sistemamodulo";
...
export function obtenerModuloConLicencia(params) {
  return from(axios.get(`${URL}/obtenerModuloConLicencia`, { params })).pipe(
    map(result => result.data.result)
  ).toPromise(); 
}

export function listarModuloPorPerfil(params) {
  return from(axios.get(`${URL}/listarModuloPorPerfil`, { params })).pipe(
    map(result => result.data.result)
  ).toPromise(); 
}
