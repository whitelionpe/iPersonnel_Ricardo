...
export const URL = Constants.API_URL + "/api/SistemaModuloAplicacion";
...
export function eliminar(params) {
  //console.log("eliminar.modulo aplicacion", params);
  return from(axios.delete(`${URL}/eliminar`, {  params })).pipe(
    map(result => result.data.result)
  ).toPromise();
}

export function listarAplicacionPorPerfil(params) {
  return from(axios.get(`${URL}/listarAplicacionPorPerfil`, { params })).pipe(
    map(result => result.data.result)
  ).toPromise();
}
