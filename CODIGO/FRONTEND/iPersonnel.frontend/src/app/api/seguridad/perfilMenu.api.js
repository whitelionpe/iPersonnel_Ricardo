...
export const URL = Constants.API_URL + "/api/SeguridadPerfilMenu";
...
export function eliminarAll(params) {
  return from(axios.delete(`${URL}/eliminarAll`, { params })).pipe(
    map(result => result.data.result)
  ).toPromise();
}

export function validarEliminar(params) {
  return from(axios.get(`${URL}/validarEliminar`, { params })).pipe(
    map(result => result.data.result)
  ).toPromise();
}
