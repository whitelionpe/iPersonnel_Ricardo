...
export const URL = Constants.API_URL + "/api/IdentificacionTipoCredencial";

export const serviceTipoCredencial = {

  obtener: (params) => {
    console.log("obtener.params",params);
    return from(axios.get(`${URL}/obtener`, { params })).pipe(
      map(result => result.data.result)
    ).toPromise();
  },

  obtenerEditBadge: (params) => {
    return from(axios.get(`${URL}/obtenerEditBadge`, { params })).pipe(
      map(result => result.data.result)
    ).toPromise();
  },
