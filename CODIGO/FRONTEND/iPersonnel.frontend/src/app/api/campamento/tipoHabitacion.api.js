...
export const URL = Constants.API_URL + "/api/CampamentoTipoHabitacion";
...
export function listarReporte(params) {
  return from(axios.get(`${URL}/listarReporte`, { params })).pipe(
    map(result => result.data.result)
  ).toPromise();
}
...
export function listarTipoHabitacionPorCampamento(params) {
  return from(axios.get(`${URL}/listarTipoHabitacionPorCampamento`, { params })).pipe(
    map(result => result.data.result)
  ).toPromise();
}