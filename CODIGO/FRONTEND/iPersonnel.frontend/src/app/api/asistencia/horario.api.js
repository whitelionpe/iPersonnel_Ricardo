...
export const URL = Constants.API_URL + "/api/AsistenciaHorario";
...
export function obtener(params) {
  ...
  export function ObtenerHorarioPorCompania(params) {
    return from(axios.get(`${URL}/ObtenerHorarioPorCompania`, { params })).pipe(
      map(result => result.data.result)
    ).toPromise();
  }
  ...
  export function obtenerTitulo(params) {
    return from(axios.get(`${URL}/obtenerTitulo`, { params: params })).pipe(
      map(result => result.data.result)
    ).toPromise();
  }
