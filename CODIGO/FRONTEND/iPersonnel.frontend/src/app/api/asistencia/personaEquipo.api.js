...
export const URL = Constants.API_URL + "/api/AsistenciaPersonaEquipo";
...
export function obtener(params) {
  ...
  export function CrearPesonaMasivo(params) {
    return from(axios.post(`${URL}/CrearPesonaMasivo`, params)).pipe(
      map(result => result.data.result)
    ).toPromise();
  }
  ...
  export function obtenerTodosPorZona(params) {
    return from(axios.get(`${URL}/obtenerTodosPorZona`, { params })).pipe(
      map(result => result.data.result)
    ).toPromise();
  }
