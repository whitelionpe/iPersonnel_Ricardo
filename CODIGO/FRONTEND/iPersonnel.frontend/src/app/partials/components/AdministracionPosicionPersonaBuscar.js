...
// AsistenciaPosicionPersonaBuscar

const AdministracionPosicionPersonaBuscar = props => {
  const { intl, selectionMode } = props;
  const [selectedRow, setSelectedRow] = useState([]);
  const { IdCliente } = useSelector((state) => state.perfil.perfilActual);

  console.log("AdministracionPosicionPersonaBuscar|props.filtro:",props.filtro);
  ...
  const { IdDivision, IdUnidadOrganizativa, IdFuncion, IdTipoPosicion, IdCompania } = props.filtro;
  ...
  useEffect(() => {
    if (props.filtro) {
      const { IdDivision, IdUnidadOrganizativa, IdFuncion, IdTipoPosicion, IdCompania } = props.filtro;
      dataSource.loadDataWithFilter({ data: { IdDivision, IdUnidadOrganizativa, IdFuncion, IdTipoPosicion, IdCompania } });
    }
  }, [props.filtro]);
    ...
    return (
      ...
      >
        ...
        <Column dataField="NombreCompleto" 
          caption={intl.formatMessage({ id: "ASSISTANCE.AUTHORIZER.ASSIGNEDTO" })}
          editorOptions={false} allowEditing={false}
          width={"30%"} />
...
AdministracionPosicionPersonaBuscar.propTypes = {
...
AdministracionPosicionPersonaBuscar.defaultProps = {
  showButton: false,
...
export default injectIntl(AdministracionPosicionPersonaBuscar);
