...
import { getDataTempLocal, isNotEmpty, setDataTempLocal, listarEstadoSimple } from "../../../_metronic";
...
import { storeBuscarPersonas as loadUrl } from "../../api/asistencia/persona.api";
...
const AsistenciaPersonaBuscar = (props) => {
...
//3.
  const valueChangedEstado = (estado) => {
    dataSource.loadDataWithFilter({ data: { Activo: estado } });//estado: "S"/"N"
  }
  ...
  const renderFormContentCustomFilter = ({ getInstance }) => {
    return (
      ...
      <GroupItem colCount={14} colSpan={14} labelLocation="top">
        <GroupItem itemType="group" colSpan={6} >
          <Item
        ...
        <GroupItem itemType="group" colSpan={5} >
          <Item
            dataField="Activo"
            colSpan={2}
            label={{ text: intl.formatMessage({ id: "ASSISTANCE.PERSON.JUSTIFICATION.ACTIVE" }) }}
...
AsistenciaPersonaBuscar.propTypes = {
...
AsistenciaPersonaBuscar.defaultProps = {
  showButton: true,
...
export default injectIntl(AsistenciaPersonaBuscar);
