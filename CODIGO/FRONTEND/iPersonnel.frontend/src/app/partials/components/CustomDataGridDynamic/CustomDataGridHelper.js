...
export const prefixForStorageKeys = "i2p:";
...
const areThereKeys = storageKey => {
  ...
  const [metadataKey, loadOptionsKey, arrayDataKey, gridInfoKey, FieldsKey, LabelsKey] = Object.values(keys).map(key => !isNullOrUndefined(localStorage.getItem(key)));
  return { metadataKey, loadOptionsKey, arrayDataKey, gridInfoKey, FieldsKey, LabelsKey };
}
...
export const getStorageKeys = storageKey => ({
  ...
  LabelsKey: `${prefixForStorageKeys}${storageKey}:dynamiclabels`,
  FieldsKey: `${prefixForStorageKeys}${storageKey}:dynamicfields`,
});
...
const getData = (response, pageSize) => {
  let { columns = [], labels = [], data = [], totalPages = 0, totalCount = 0 } = response;
  ...
  return { data, totalPages, totalCount, labels, fields: columns }
}
export const performLoad = (
  ...
  try {
    ...
    const { loadOptionsKey, arrayDataKey, gridInfoKey, FieldsKey, LabelsKey } = getStorageKeys(storageKey);
      ...
      return axios
        ...
        .then(response => {
          ...
          if (!(response instanceof Error)) {
            ...
            let { data, totalPages, totalCount, labels, fields } = getData(response, take); //EGSC
            // console.log("data, totalPages, totalCount---->", { data, labels, fields, totalPages, totalCount });
            ...
            if (!clearStoredStateIfResultHasNoData || (clearStoredStateIfResultHasNoData && totalCount !== 0 && data.length !== 0)) {
              ...
              localStorage.setItem(FieldsKey, JSON.stringify({ fields }));//EGSC AGREGAR
              localStorage.setItem(LabelsKey, JSON.stringify({ labels }));//EGSC AGREGAR 
            } else {
              localStorage.removeItem(arrayDataKey);
              localStorage.removeItem(gridInfoKey);
              localStorage.removeItem(FieldsKey);//EGSC
              localStorage.removeItem(LabelsKey);//EGSC
            }
            return { data, LabelsKey: labels, FieldsKey: fields, totalPages, totalCount, changeCurrentPage, dataLoaded: dataLoadTypes.FromServer, params: extendedOptions, queryString: params, delayTime: getDelayTime(ts) };
          } else {
            throw response;
          }
        })
    ...
    if (goToStorage) {
      ...
      if (ifThereAreNoChangesLoadFromStorage || forceLoad === forceLoadTypes.FromLocalStorage) {
        ...
        let FieldsKeyStorage = JSON.parse(localStorage.getItem(FieldsKey) || '[]');//EGSC AGREGAR
        let LabelsKeyStorage = JSON.parse(localStorage.getItem(LabelsKey) || '[]');//EGSC AGREGAR

        // console.log({ arrayDataKey, FieldsKey, LabelsKey, data, FieldsKeyStorage });
        ...
        const target = { data, FieldsKey: FieldsKeyStorage, LabelsKey: LabelsKeyStorage, totalPages, totalCount, changeCurrentPage: false, dataLoaded: dataLoadTypes.FromLocalStorage, params: undefined, queryString: undefined };
        const ts = new Date().getTime();
        ...
        // console.log("-->2 ");
        const target = { data: [], FieldsKey: [], LabelsKey: [], totalPages: 0, totalCount: 0, changeCurrentPage: false, dataLoaded: dataLoadTypes.FromNoSource, params: undefined, queryString: undefined };
        const ts = new Date().getTime();
        return createObservable(target)
          .toPromise()
          .then(response => {
