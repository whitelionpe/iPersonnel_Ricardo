...
const HabitacionExclusivaEditPage = (props) => {
  ...
  function grabar(e) {
     ...
     if (result.isValid) {
       if (props.dataRowEditNew.esNuevoRegistro) {
         props.agregarHabitacionExclusiva(props.dataRowEditNew);
       } else { 
         props.actualizarHabitacionExclusiva(props.dataRowEditNew);
       }
     }
  }
  ...
  const agregarPersonaCamaExclusiva = async (personas) => {
    ...
    props.setDataRowEditNew({ ...props.dataRowEditNew, IdPersona, NombreCompleto });
     ...
     await servicePersona.obtenerPeriodo({
      ...
      console.log("agregarPersonaCamaExclusiva|response:",response);
        if(response){
                ...
                <AppBar position="static" className={classesEncabezado.secundario}>
                  <Toolbar variant="dense" className={classesEncabezado.toolbar}>
                    <Typography variant="h6" color="inherit" className={classesEncabezado.title}>
                      {intl.formatMessage({ id: "CAMP.EXCLUSIVE.ROOM.ADD" })}
                    </Typography>
                  </Toolbar>
                </AppBar>
              </Item>
...
export default injectIntl(WithLoandingPanel(HabitacionExclusivaEditPage));
