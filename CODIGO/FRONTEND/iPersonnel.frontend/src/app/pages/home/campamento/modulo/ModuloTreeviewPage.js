...
import { listarTreeView } from "../../../../api/campamento/modulo.api";
...
const ModuloTreeViewPage = (props) => {
    ...
    const listar = async () => {
        const { IdCliente, IdDivision, IdCampamento, IdModulo } = data;
        const params = {
            idCliente: IdCliente,
    ...
    const seleccionarNodo = (selectedNode) => {
        if (!timeout) {
            ...
            if (selectedNode.Nivel === 2) return;
            if (seleccionado === selectedNode) {
                props.cambiarTab(selectedNode, "modulo");
            }
        }
    }
...
export default injectIntl(WithLoandingPanel(ModuloTreeViewPage));