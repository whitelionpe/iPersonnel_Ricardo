...
import { isNotEmpty, listarEstadoSimple, listarTipoHabitacion } from "../../../../../_metronic";
import CustomDataGrid from "../../../../partials/components/CustomDataGrid";
import { forceLoadTypes } from "../../../../partials/components/CustomDataGrid/CustomDataGridHelper";
import { storeListar as loadUrl } from "../../../../api/campamento/habitacion.api";
import { initialFilter } from "./HabitacionExclusivaIndexPage";
...
const HabitacionListPage = props => {
  const cssBackgroundGreen = useStyles();
  const { intl } = props;
  ...
  useEffect(() => {
    if (servicioSeleccionados) {
      ...
      props.dataSource.loadDataWithFilter({ data: { IdServicio: strServicios } });
  ...
  const cellEstadoCamaRender = (e) => {
    ...
    let css = '';
    switch (e.text) {
      case 'L': estado = "Libre"; break;
      case 'R': css = 'item_reservado'; estado = "Reservado"; break;
      case 'O': css = 'item_ocupado'; estado = "Ocupado"; break;
    };

    return (css === '') ?
      <span className={css} style={css_caja_estado_negro}  >{estado}</span>
      : <span className={css} style={css_caja_estado}  >{estado}</span>
  }
  ...
  function cellRenderImage(param) {
    if (param && param.data) {
      const { StatusExclusividad,RowIndex } = param.data;
      return (StatusExclusividad === 'S') ?
        <>
          <i className="fas fa-circle  text-success icon-10x" id={`id_${RowIndex}`} ></i>
          <Tooltip
            target={`#id_${RowIndex}`}
          ...
          >
            <div><p>{intl.formatMessage({ id: "CAMP.EXCLUSIVE.AVAILABLE" })}</p></div>
          </Tooltip>
        </>
        :
        <>
          ...
          <Tooltip
            ...
            hideOnOutsideClick={false}
          >
            <div><p>{intl.formatMessage({ id: "CAMP.EXCLUSIVE.NOTAVAILABLE" })}</p></div>
          </Tooltip>
        </>
    }
  }
    ...
    return (
      ...
      >
        ...
        <Column dataField="IdHabitacion"
          caption={intl.formatMessage({ id: "Código Habitacion " })}
          visible={false}
        />
        ...
        <Column
          dataField="CantidadCamasAsignadas"
          caption={intl.formatMessage({ id: "CAMP.BEDROOM.TOT" })}
          alignment={"center"}
          width={"15%"}
          allowSorting={false}
          allowFiltering={false}
        ...
        <Column
          dataField="CantidadPersonasAsignadas"
          caption={intl.formatMessage({ id: "CAMP.BEDRROM.TOT.RESERVATION" })}
          alignment={"center"}
          width={"15%"}
          allowSorting={false}
          allowFiltering={false}
    ...
    <>
      ...
      <PortletBody>
        ...
        <CustomDataGrid
          ...
          initialLoadOptions={{ currentPage: 1, pageSize: 20, sort: { column: 'Habitacion', order: 'asc' } }}
          filterRowSize='sm'
        ...
        <CampamentoPerfilBuscar
          ...
          uniqueId={"campamentoPerfilHabitacionListPage"}
        />
...
HabitacionListPage.defaultProps = {
  ...
  uniqueId: 'HabitacionList',
  selected: { IdDivision: "" }
}

export default injectIntl(HabitacionListPage);
