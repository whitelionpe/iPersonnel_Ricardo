...
import { storeListarDesmovilizacionPersona as loadUrl, serviceSolicitud } from "../../../../../../api/acreditacion/solicitud.api";
import { initialFilter } from "./DesmovilizacionPersonaIndexPage";
...
const DesmovilizacionPersonaListPage = props => {
  ...
  const exportReport = async () => {
    ...
    const { IdCliente, IdCompaniaContratista, IdDivision, IdSolicitud, Documento, NombreCompleto, CompaniaMandante, EstadoAprobacion,
      FechaInicio, FechaFin, IdCompaniaMandante, IdContrato, EsSubContratista, EstadoSolicitud, IdMotivoCese, UnidadesOrganizativas,
    ...
    if (dataGridRef.current.props.dataSource._items.length > 0) {
      ...
      setLoading(true);
      await serviceSolicitud.exportarExcelDesmovilizacionPersona(params).then(response => {
    ...
    return (
      <>
        ...
        >
          ...
          <Column
            dataField="FechaCese"
            caption={intl.formatMessage({ id: "ADMINISTRATION.CESSATION.DATE" })}
            //dataType="date"
            //format="dd/MM/yyyy"
            alignment={"center"}
            allowHeaderFiltering={false}
...
DesmovilizacionPersonaListPage.propTypes = {
...
DesmovilizacionPersonaListPage.defaultProps = {
  titulo: "",
  modoEdicion: false,
  showButtons: true,
  showAppBar: true,
...
export default injectIntl(WithLoandingPanel(DesmovilizacionPersonaListPage));
