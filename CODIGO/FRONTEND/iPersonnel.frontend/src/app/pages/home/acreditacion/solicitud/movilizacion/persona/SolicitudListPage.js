...
import { isNotEmpty, listarEstado, listarEstadoAprobacion, STATUS_ACREDITACION_SOLICITUD, dateFormat, convertyyyyMMddToDate } from "../../../../../../../_metronic";
...
import { storeListarGeneral as loadUrl, serviceSolicitud } from "../../../../../../api/acreditacion/solicitud.api";
import { initialFilter } from "./SolicitudIndexPage";
...
const SolicitudListPage = props => {
  ...
  async function cargarCombos() {
    ...
    //filtrar los perfiles de entidad P:Persona y tipo operacion "MOVILIZACION" y visita "NO"
    setPerfil(perfil.filter(find => find.IdEntidad === 'P' && find.TipoOperacion === "M" && find.Visita === "N"));
  ...
  const exportReport = async () => {
    ...
    if (dataGridRef.current.props.dataSource._items.length > 0) {
      ...
      setLoading(true);
      await serviceSolicitud.exportarExcel(params).then(response => {
      ...
      <GroupItem>
        <GroupItem itemType="group" colCount={2} colSpan={2}>
          ...
          <Item
            ...
            label={{
              text: intl.formatMessage({ id: "ACCESS.PERSON.REQUEST.STARTDATE" }),
            }}
            isRequired={true}
          ...
          <Item
            dataField="FechaFin"
            label={{
              text: intl.formatMessage({ id: "ACCESS.PERSON.REQUEST.ENDDATE" }),
            }}
            isRequired={true}
            editorType="dxDateBox"
            dataType="datetime"
...
SolicitudListPage.defaultProps = {
  titulo: "",
  modoEdicion: false,
  showButtons: true,
  showAppBar: true,
...
export default injectIntl(WithLoandingPanel(SolicitudListPage));
