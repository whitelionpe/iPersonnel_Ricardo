...
import { storeListarActualizacionVehiculo as loadUrl, serviceSolicitud } from "../../../../../../api/acreditacion/solicitud.api";
import { initialFilter } from "./ActualizacionVehiculoIndexPage";
...
const ActualizacionVehiculoListPage = props => {
  ...
  const exportReport = async () => {
    ...
    const { IdCliente, IdCompaniaContratista, IdDivision, IdSolicitud, Placa, TipoVehiculo, CompaniaMandante, EstadoAprobacion,
    ...
    if (dataGridRef.current.props.dataSource._items.length > 0) {
      ...
      setLoading(true);
      await serviceSolicitud.exportarExcelActualizacionVehiculo(params).then(response => {
      ...
      <>
        ...
        >
          ...
          <Column
            ...
            caption={intl.formatMessage({ id: "ADMINISTRATION.CONTRACT.TYPEVEHICLE" })}
            allowSorting={true}
            allowHeaderFiltering={false}
            width={"20%"}
          />
...
ActualizacionVehiculoListPage.propTypes = {
...
ActualizacionVehiculoListPage.defaultProps = {
  titulo: "",
  modoEdicion: false,
  showButtons: true,
  showAppBar: true,
...
export default injectIntl(WithLoandingPanel(ActualizacionVehiculoListPage));
