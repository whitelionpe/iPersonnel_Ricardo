...
import { storeListarSolicitudVehiculo as loadUrl, serviceSolicitud } from "../../../../../../api/acreditacion/solicitud.api";
import { initialFilter } from "./MovilizacionVehiculoIndexPage";
...
const MovilizacionVehiculoListPage = props => {
  ...
  async function cargarCombos() {
    ...
    let perfil = await service.listar({ IdCliente });
    //filtrar los perfiles de entidad P:Vehiculo y tipo operacion "MOVILIZACION" y visita "NO"
    setPerfil(perfil.filter(find => find.IdEntidad === 'V' && find.TipoOperacion === "M" && find.Visita === "N"));
  ...
  const exportReport = async () => {
    ...
    const { IdCliente, IdCompaniaContratista, IdDivision, IdSolicitud, Placa, CompaniaMandante, EstadoAprobacion,
      FechaInicio, FechaFin, IdCompaniaMandante, IdContrato, EsSubContratista, EstadoSolicitud, IdPerfil, UnidadesOrganizativas,
      Procesado } = filterData;
    ...
    if (dataGridRef.current.props.dataSource._items.length > 0) {
      ...
      let params = {
        ...
        TituloHoja: intl.formatMessage({ id: "ACTION.EXPORT" }) + intl.formatMessage({ id: "MOVILIZACIÓN VEHÍCULOS" }),
        NombreHoja: intl.formatMessage({ id: "MOVILIZACIÓN VEHÍCULOS" }),
        ArrayColumnHeader,
        ArrayColumnId,
        OrderField: selector
      };
      setLoading(true);
      await serviceSolicitud.exportarExcelSolicitudVehiculo(params).then(response => {
    ...
    return (
      <>
        ...
        >
          ...
          {/* <Column
            dataField="Serie"
            caption={intl.formatMessage({ id: "Serie" })}
...
MovilizacionVehiculoListPage.propTypes = {
...
MovilizacionVehiculoListPage.defaultProps = {
  titulo: "",
  modoEdicion: false,
  showButtons: true,
  showAppBar: true,
  showHeaderInformation: false,
  uniqueId: 'solicitudVehiculoList',
  selected: { IdDivision: "" }
}

export default injectIntl(WithLoandingPanel(MovilizacionVehiculoListPage));
