...
const ImportarDatosIndexPage = (props) => {
  ...
  const [dataRowEditNew, setDataRowEditNew] = useState({ Tabla: "ADMINISTRACION_IMPORTAR_PERSONA" });
  ...
  async function eventDownloadHeaders(e) {
    ...
    await service.descargarPlantillaPersona({ Tabla })
      .then(resp => {
        if (resp.error === 0) {
          let temp = `data:application/vnd.ms-excel;base64,${encodeURIComponent(resp.fileBase64)}`;

          console.log("eventDownloadHeaders|temp:", temp);
    ...
    if (inputFile.files.length > 0) {
      ...
      if (x != "" && x != undefined) {
        ...
        if (archivo != "") {
          ...
          setLoading(true);
          await service.cargarDatosPersona({
            ...param,
            File: archivo
          })
            .then(async res => {
    ...
    return <>
      ...
      >
        <Editing
          ...
          texts={textEditing}
        />
        <Column dataField="Documento" caption={intl.formatMessage({ id: "ADMINISTRATION.MASIVO.GRID.DOC" })} width={"15%"} alignment={"center"} />
        <Column dataField="Apellido" caption={intl.formatMessage({ id: "ADMINISTRATION.MASIVO.GRID.APELLIDO" })} width={"15%"} alignment={"left"} />
        <Column dataField="Nombre" caption={intl.formatMessage({ id: "ADMINISTRATION.MASIVO.GRID.NOMBRE" })} width={"15%"} alignment={"left"} />
