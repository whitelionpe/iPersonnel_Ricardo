...
const CambiarPersonaContratoEditPage = (props) => {
  ...
  const [finalizarCambioContrato, setFinalizarCambioContrato] = useState(false);
  ...
  const agregarContrato = (contrato) => {
    ...
    //console.log("agregarContrato",contrato);

    const { IdContrato, FechaInicio, FechaFin, IdCompaniaMandante, IdCompaniaContratista } = contrato[0];
    ...
    if (isNotEmpty(IdContrato)) {
      ...
      //Actualizar filtros de nuevo contrato..
      setLocalFilter({
        IdCliente: props.dataContract.IdCliente,
        IdContrato: contrato[0].IdContrato,
        //IdCompaniaMandante: IdCompaniaMandante,
  ...
  const selectUnidadOrganizativa = (dataPopup) => {
    ...
    //Listar centro de costo asociado a la unidad organizativa..
  ...
  function grabar(e) {
    ...
    //validar que el contrato nuevo sea del misma compania
    if (!(//formDataContract.IdCompaniaMandante == IdCompaniaMandante &&
      formDataContract.IdCompaniaContratistaPrevia == formDataContract.IdCompaniaContratista)) {
      handleInfoMessages(intl.formatMessage({ id: "ADMINISTRATION.PERSON.VALIDATION.MESSAGE.SAME.COMPANY" }));
      return;
    }
    //validar que el contrato nuevo sea distinto al actual
    if (formDataContract.IdContratoPrevia == formDataContract.IdContrato) {
      handleInfoMessages(intl.formatMessage({ id: "ADMINISTRATION.PERSON.VALIDATION.MESSAGE.DIFERENT.CONTRACT" }));
      return;
    }
    //Validar que la fecha Fin del Contrato nuevo NO sea menor a la fecha de hoy
    if (Date.parse(new Date(formDataContract.FechaFin)) < Date.parse(new Date(getDateOfDay().FechaInicio))) {
      handleInfoMessages(intl.formatMessage({ id: "ADMINISTRATION.PERSON.VALIDATION.MESSAGE.FINALDATE.CONTRACT" }));
      return;
    }
    //Validar que la fecha Fin del Contrato nuevo NO sea menor a la fecha de hoy
    if (Date.parse(new Date(formDataContract.FechaInicio)) > Date.parse(new Date(getDateOfDay().FechaInicio))) {
      handleInfoMessages(intl.formatMessage({ id: "ADMINISTRATION.PERSON.VALIDATION.MESSAGE.INITIALDATE.CONTRACT" }));
      return;
    }

    let result = e.validationGroup.validate();
    if (result.isValid) {
      //validar que la fecha de Fin sea mayor a la fecha de Inicio de contrato
  ...
  async function actualizarDatosContrato(datos) {
    ...
    await servicePersonaContrato.cambiarContratoPersona(params).then(result => {
      setResultadoProceso(result);
      setFinalizarCambioContrato(true);
                ...
                toolbar={
                  <PortletHeaderToolbar>
                    ...
                    <Button
                      ...
                      disabled={finalizarCambioContrato}
                    />
                    &nbsp;
                    <Button
...
export default injectIntl(WithLoandingPanel(CambiarPersonaContratoEditPage)); 
