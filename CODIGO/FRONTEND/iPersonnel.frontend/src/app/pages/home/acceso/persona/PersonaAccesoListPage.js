...
import { storeListar as loadUrl, servicePersona } from "../../../../api/acceso/persona.api";
import { initialFilter } from "./PersonaAccesoIndexPage";
...
const PersonaAccesoListPage = props => {
  ...
  useEffect(() => {
    if (IdDivision) {
      props.dataSource.loadDataWithFilter({ data: { IdDivision, IdValue: new Date() } });
    }
  }, [IdDivision]);
...
PersonaAccesoListPage.propTypes = {
...
PersonaAccesoListPage.defaultProps = {
  titulo: "",
  modoEdicion: false,
  showButtons: true,
  showAppBar: true,
...
export default injectIntl(WithLoandingPanel(PersonaAccesoListPage));
