...
import PersonalNoAbordoFilter from './PersonalNoAbordoFilter';
...
const PersonalNoAbordo = (props) => {
  ...
  const [tabs, setTabs] = useState([{
    ...
    bodyRender: (e) => { return <PersonalNoAbordoFilter {...e} /> }
  }]);
  ...
  const cargarReporte = async (e) => {
    ...
    let {FechaDesde, FechaHasta,Rutas,IdPersona,IdCompania } = filterData;
    FechaDesde = dateFormat(FechaDesde, 'yyyyMMdd');
    FechaHasta = dateFormat(FechaHasta, 'yyyyMMdd hh:mm');

    let enviarParametros = {
      FechaDesde,
      FechaHasta,
      RutasString: Rutas,
      CodigoPasajero:IdPersona,
      CodigoCompania:IdCompania, 
    ...
    let item = {
      ...
      bodyRender: (e) => {
        return <FrameReport
        // ReportName={'PersonalNoAbordo'}
        ReportName={'TRANSPORTE_004_PERSONAS_NO_ABORDO'}
        ReportDescription={''}
    ...
    <>
      <div className="row">
        <div className="col-md-12">
          <CustomBreadcrumbs Title={intl.formatMessage({ id: "TRANSPORT.MAIN" })} SubMenu={intl.formatMessage({ id: "CAMP.REPORT.REPORTS" })} Subtitle={intl.formatMessage({ id: "CONFIG.MENU.TRANSPORTE.PERSONAL_QUE_NO_ABORDO" })} />
          <Portlet>
            <AppBar position="static" className={classes.principal}>
              <Toolbar variant="dense" className={classes.toolbar}>
                <Typography variant="h6" color="inherit" className={classes.title}>
                  {intl.formatMessage({ id: "CONFIG.MENU.TRANSPORTE.PERSONAL_QUE_NO_ABORDO" })}
                  </Typography>
              </Toolbar>
            </AppBar>
...
export default injectIntl(WithLoandingPanel(PersonalNoAbordo));
