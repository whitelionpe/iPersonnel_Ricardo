...
const HorarioMasivoFormImportPage = (props) => {
    ...
    if (props.dataPersonasTemporal.length > 0) {
      ...
      if (personas.length > 0) {
        ...
        if (response.isConfirmed) {
          ...
          let listPersonas = [];
          personas.map(item =>
            listPersonas.push({ ...item, FechaInicio: dateFormat(item.FechaInicio, 'yyyyMMdd'), FechaFin: dateFormat(item.FechaFin, 'yyyyMMdd') })
          );
          console.log("procesarHorario->person->", personas);
          console.log("procesarHorario->listPersonas", listPersonas);
          props.procesarPersonas(listPersonas);
        }
      }
      else {
  ...
  async function descargarPlantilla(e) {
    ...
    setLoading(true);
    await serviceHorarioMasivo.descargarPlantilla({ IdDivision, IdCompania: varIdCompania, Compania: companyName, FileName: "ImportarHorarios" })
      .then(resp => {
        if (resp.error === 0) {
  ...
  const importarHorarioExcel = async (e) => {
    ...
    setLoading(true);
    await serviceHorarioMasivo.importarHorarios({
      ...param,
      File: archivo
    }).then(async res => {
...
export default injectIntl(WithLoandingPanel(HorarioMasivoFormImportPage));
