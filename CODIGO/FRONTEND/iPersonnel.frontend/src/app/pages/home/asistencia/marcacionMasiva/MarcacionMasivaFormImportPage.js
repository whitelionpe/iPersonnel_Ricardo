...
import { servicioMarcacionMasiva } from "../../../../api/asistencia/marcacionMasiva.api";

const MarcacionMasivaFormImportPage = (props) => {
  ...
  const procesarJustificacion = async (e) => {
    ...
    if (props.dataPersonasTemporal.length > 0) {
      ...
      if (personas.length > 0) {
        ...
        var response = await confirmAction(intl.formatMessage({ id: "ASSISTANCE.MASSIVE.MARK.MSG.REPLACE" }), intl.formatMessage({ id: "COMMON.YES" }), intl.formatMessage({ id: "COMMON.NOT" }));
  ...
  async function descargarPlantilla(e) {
    ...
    setLoading(true);
    await servicioMarcacionMasiva.descargarPlantilla({ IdDivision, IdCompania: varIdCompania, Compania: companyName, FileName: "ImportarMarcaciones" })
      .then(resp => {
        if (resp.error === 0) {
  ...
  const importarJustificacionExcel = async (e) => {
    ...
    setLoading(true);
    await servicioMarcacionMasiva.importarMarcaciones({
      ...param,
      File: archivo
    }).then(async res => {
...
export default injectIntl(WithLoandingPanel(MarcacionMasivaFormImportPage));
