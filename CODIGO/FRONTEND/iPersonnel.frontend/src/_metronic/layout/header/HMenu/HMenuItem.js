...
export default class HMenuItem extends React.Component {
  ...
  getItemMenuSubmenuClass = item => {
    ...
    const c_alignment = objectPath.get(item, "alignment") || "right";
    let { Nivel, alignment } = item;
    ...
    // console.log("Jimmy>objectPath>1>",objectPath.get(item, "width"));
    // console.log("Jimmy>objectPath>2>", objectPath.get(item, "type"));

    // console.log("Jimmy->item->",item);
    // console.log("Jimmy->Class->SUB->",classes );
    return classes;
  };
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {!item.submenu && (
          <Link to={{ pathname: `/${item.page || ""}` }} className="kt-menu__link">
            <MenuItemInner
              item={item}
              parentItem={parentItem}
              currentUrl={currentUrl}
